 ...
 *section is given a value "false". The two sections are then put through a nor gate, and if the
...
int SwitchIn_LEDOut(void)
{
	...
	int or = (section1+section2);
	if(or == 2){or = 1;}
	printf("\nSection 1 AND section 2 gives: %d", or);
	...
	printf("The LED configuration is: %d. ", nor);
	...
	XGpio_DiscreteWrite(&Gpio, LED_CHANNEL, nor);
